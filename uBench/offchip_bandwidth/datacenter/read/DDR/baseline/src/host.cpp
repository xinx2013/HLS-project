...
int main(int argc, char *argv[]) {
    ...
    for (int payload(32768); payload <= 32768; payload*=2){
        ...
        std::cout << "//*************************************************************//" << std::endl;
        std::cout << "//*************************************************************//" << std::endl;
        std::cout << "THIS IS baseline, configured with 1 rank." << std::endl;
	...
	//DRAM (DDR4) power estimation
        double ph = (1 - (double)stride/64);
        double activation_power = (125.8) * (1 - ph)/0.9;
        double background_power = 72.8;
        double read_write_power = 110.2;

	if (ph <= 0){
	    ph = 1/64;
            activation_power = (125.8) * (1 - ph)/0.9;
	}
